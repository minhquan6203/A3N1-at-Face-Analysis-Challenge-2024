...
class Multi_Task_Model_Hydra(nn.Module):
    def __init__(self,config: Dict):

        super(Multi_Task_Model_Hydra, self).__init__()
        self.intermediate_dims = config["model"]["intermediate_dims"]
        self.encoder_age = UniModalEncoder(config)
        self.encoder_race = UniModalEncoder(config)
        self.encoder_masked = UniModalEncoder(config)
        self.encoder_skintone = UniModalEncoder(config)
        self.encoder_emotion = UniModalEncoder(config)
        self.encoder_gender = UniModalEncoder(config)
    ...
    def forward(self, images: List[str], age = None, race = None, masked = None,
        ...
        encoded_feature_age = self.encoder_age(embedded_vision, vison_mask)
        encoded_feature_race = self.encoder_race(embedded_vision, vison_mask)
        encoded_feature_masked = self.encoder_masked(embedded_vision, vison_mask)
        encoded_feature_skintone = self.encoder_skintone(embedded_vision, vison_mask)
        encoded_feature_emotion= self.encoder_emotion(embedded_vision, vison_mask)
        encoded_feature_gender = self.encoder_gender(embedded_vision, vison_mask)

        head_age=F.log_softmax(torch.mean(self.classifier_age(encoded_feature_age),dim=1),dim=-1)
        head_race=F.log_softmax(torch.mean(self.classifier_race(encoded_feature_race),dim=1),dim=-1)
        head_masked=F.log_softmax(torch.mean(self.classifier_masked(encoded_feature_masked),dim=1),dim=-1)
        head_skintone=F.log_softmax(torch.mean(self.classifier_skintone(encoded_feature_skintone),dim=1),dim=-1)
        head_emotion=F.log_softmax(torch.mean(self.classifier_emotion(encoded_feature_emotion),dim=1),dim=-1)
        head_gender=F.log_softmax(torch.mean(self.classifier_gender(encoded_feature_gender),dim=1),dim=-1)
